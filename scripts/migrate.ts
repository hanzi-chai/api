...
const 旧字符数据列表: CharacterModel[] = JSON.parse(
	readFileSync("data/repertoire.json", "utf-8"),
);

type IDSource = { id: number; source: string[] };
...
interface 复合体数据 extends 字形数据基础 {
	type: "compound";
	operator: Operator;
	references: 引用数据[];
	strokes?: 引用笔画块数据[];
...
type 字形数据 = 部件数据 | 复合体数据;

// 构建 unicode -> Glyph[] 的映射
const charMap = new Map<number, Glyph[]>();
...
const resolvedMap = new Map<number, IDSource[]>();
	...
	while (i < strokes.length) {
		...
		if (stroke.feature === "reference") {
			...
			while (
				...
				(strokes[i] as 引用笔画数据).index === to + 1
			) {
				to = (strokes[i] as 引用笔画数据).index;
				i++;
			}
...
function resolve(
	...
	// 已经解析过，直接返回缓存结果
	if (resolvedMap.has(unicode)) {
		return resolvedMap.get(unicode)!;
	}
